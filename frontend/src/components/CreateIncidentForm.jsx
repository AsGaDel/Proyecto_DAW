...
function inputClass(touched, error) {
  ...
  return `${baseInput} border-gray-200 focus:ring-blue-500 hover:border-blue-400 transition-colors`;
}
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-8">
      ...
      <div className="flex-1">
        <Field label="Imagen" error={touched.photo && errors.photo}>
          <ImageUploader
            value={form.photo}
            onChange={(file) => {
              const updatedForm = { ...form, photo: file };
              setForm(updatedForm);
              if (touched.photo) setErrors(validate(updatedForm));
            }}
          />
        </Field>
