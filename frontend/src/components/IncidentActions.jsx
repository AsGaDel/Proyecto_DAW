...
export default function IncidentActions({ incidentId }) {
  ...
  return (
    <div className="flex items-center gap-3 pt-2">
      ...
      {/* Votar */}
      <button onClick={handleVote} className={`flex flex-col sm:flex-row items-center gap-2 px-4 py-2.5 rounded-lg text-xs sm:text-sm font-semibold border transition-all duration-150
          ${voted
      ...
      {/* Suscribirse */}
      <button onClick={handleSubscribe} className={`flex flex-col sm:flex-row items-center gap-2 px-4 py-2.5 rounded-lg text-xs sm:text-sm font-semibold border transition-all duration-150
          ${subscribed
      ...
      <button onClick={() => setShareOpen(true)} 
              className="flex flex-col sm:flex-row  items-center gap-2 px-4 py-2.5 rounded-lg text-xs sm:text-sm font-semibold border transition-all duration-150
               bg-gray-200 text-gray-700 border-gray-300 hover:border-green-400 hover:text-green-600">
...
export default function IncidentActions({ incidentId, initialVoted = false, initialSubscribed = false, initialVotes = 0 }) {
  ...
  const handleSubscribe = async () => {
    ...
    setLoading(true);
    try {
      await incidentService.subscribe(incidentId);
      const newSubscribed = !subscribed;
      setSubscribed(newSubscribed);
      toast({
        message: newSubscribed ? "Suscrito a este incidente." : "Te has desuscrito del incidente.",
        type: "success",
      });
    } catch (err) {
  ...
  return (
    <div className="flex items-center gap-3 pt-2">
      ...
      <button
        onClick={handleVote}
        disabled={loading}
        className={`flex flex-col sm:flex-row items-center gap-2 px-4 py-2.5 rounded-lg text-xs sm:text-sm font-semibold border transition-all duration-150 disabled:opacity-60
          ${voted
            ? "bg-blue-600 text-white border-blue-600 hover:bg-blue-700"
            : "bg-gray-200 text-gray-700 border-gray-300 hover:border-blue-400 hover:text-blue-600"
          }`}
